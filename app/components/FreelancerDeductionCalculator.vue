<template>
  <div class="w-full max-w-2xl mx-auto space-y-6">
    <form class="bg-zinc-900 rounded-xl border border-zinc-800 p-6 space-y-5" @submit.prevent>
      <!-- Income -->
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div class="sm:col-span-2">
          <label class="block text-sm font-medium text-zinc-300 mb-1" for="grossIncome">
          ...
          />
          <p class="text-xs text-zinc-500 mt-1">Before business expenses and taxes</p>
        </div>

        <div>
          <label class="block text-sm font-medium text-zinc-300 mb-1" for="filingStatus">Filing Status</label>
        ...
        <div>
          <label class="block text-sm font-medium text-zinc-300 mb-1" for="state">State</label>
      ...
      <div class="border-t border-gray-100 pt-5">
        <p class="text-sm font-medium text-zinc-300 mb-4">Business Deductions</p>

        <div class="space-y-4">
          <!-- Home office -->
          <div>
            <label class="block text-sm font-medium text-zinc-300 mb-1">Home Office</label>
            <select v-model="homeOfficeMethod"
              class="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 text-zinc-100 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 mb-2">
            ...
            <input
              ...
              class="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 text-zinc-100 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            />
            <p v-if="homeOfficeMethod === 'simplified'" class="text-xs text-zinc-500 mt-1">Square feet dedicated to business (max 300)</p>
            <input
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-zinc-300 mb-1" for="businessMiles">
                Business Miles
              </label>
              <input id="businessMiles" v-model.number="businessMiles"
                ...
                class="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 text-zinc-100 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
              <p class="text-xs text-zinc-500 mt-1">$0.67/mi (2026 IRS rate)</p>
            </div>

            <div>
              <label class="block text-sm font-medium text-zinc-300 mb-1" for="equipment">
                Equipment & Supplies ($)
              </label>
      ...
      <!-- Above-the-line -->
      <div class="border-t border-gray-100 pt-5">
        <p class="text-sm font-medium text-zinc-300 mb-4">Above-the-Line Deductions</p>

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label class="block text-sm font-medium text-zinc-300 mb-1" for="healthInsurance">
            ...
            <input id="healthInsurance" v-model.number="healthInsurance"
              ...
              class="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 text-zinc-100 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            />
            <p class="text-xs text-zinc-500 mt-1">Self-employed: 100% deductible</p>
          </div>

          <div>
            <label class="block text-sm font-medium text-zinc-300 mb-1">Retirement Account</label>
            ...
            <input
              ...
              class="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 text-zinc-100 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            />
            <p v-if="result && retirementType !== 'none'" class="text-xs text-zinc-500 mt-1">
              Your cap: {{ fmt(result.retirementCap) }}
            </p>
          </div>

          <div class="sm:col-span-2">
            <label class="block text-sm font-medium text-zinc-300 mb-1" for="other">
              Other Deductions ($)
            </label>
    ...
    <div v-if="result && grossIncome > 0" class="space-y-4">
      ...
      <div class="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-5 text-center">
        <p class="text-sm text-emerald-400 mb-1">Total Tax Savings</p>
        <p class="text-4xl font-bold text-emerald-300">{{ fmt(result.taxSaved) }}</p>
        <p class="text-sm text-emerald-500 mt-1">{{ fmt(result.totalDeductions) }} in deductions</p>
      </div>
      ...
      <div class="bg-zinc-900 rounded-xl border border-zinc-800 overflow-hidden">
        <div class="grid grid-cols-2 divide-x divide-zinc-800">
          <div class="p-5">
            <p class="text-xs font-semibold text-zinc-500 uppercase tracking-wide mb-3">Without Deductions</p>
            <div class="space-y-2 text-sm">
              ...
              <div v-if="result.before.stateTax > 0" class="flex justify-between text-zinc-400">
                <span>State Tax</span><span>{{ fmt(result.before.stateTax) }}</span>
              </div>
              <div class="flex justify-between font-semibold text-zinc-100 border-t border-zinc-800 pt-2">
                <span>Total Tax</span><span>{{ fmt(result.before.totalTax) }}</span>
              </div>
              <div class="flex justify-between font-semibold text-zinc-100">
                <span>Net Income</span><span>{{ fmt(result.before.netIncome) }}</span>
              </div>
              <div class="flex justify-between text-zinc-600 text-xs">
          ...
          <div class="p-5 bg-green-50/40">
            <p class="text-xs font-semibold text-emerald-400 uppercase tracking-wide mb-3">With Deductions</p>
            <div class="space-y-2 text-sm">
              <div class="flex justify-between text-zinc-400">
              ...
              <div v-if="result.after.stateTax > 0" class="flex justify-between text-zinc-400">
        ...
        <div class="border-t border-zinc-800 p-5">
          <p class="text-xs font-semibold text-zinc-500 uppercase tracking-wide mb-3">Deduction Breakdown</p>
          <div class="space-y-1.5 text-sm">
            <div v-if="result.homeOfficeDeduction > 0" class="flex justify-between text-zinc-400">
              <span>Home Office</span><span>{{ fmt(result.homeOfficeDeduction) }}</span>
            </div>
            <div v-if="result.mileageDeduction > 0" class="flex justify-between text-zinc-400">
              <span>Mileage</span><span>{{ fmt(result.mileageDeduction) }}</span>
            </div>
            <div v-if="equipment > 0" class="flex justify-between text-zinc-400">
              <span>Equipment & Supplies</span><span>{{ fmt(equipment) }}</span>
            </div>
            <div v-if="result.aboveLineTotal > 0" class="flex justify-between text-zinc-400">
              <span>Health Insurance + Retirement</span><span>{{ fmt(result.aboveLineTotal) }}</span>
            </div>
            <div v-if="otherDeductions > 0" class="flex justify-between text-zinc-400">
