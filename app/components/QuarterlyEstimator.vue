  ...
  <div class="w-full max-w-2xl mx-auto">
    <form class="bg-zinc-900 rounded-xl border border-zinc-800 p-6 space-y-4" @submit.prevent>
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label class="block text-sm font-medium text-zinc-300 mb-1" for="annualIncome">
          ...
          />
          <p class="text-xs text-zinc-500 mt-1">After business expenses</p>
        </div>

        <div>
          <label class="block text-sm font-medium text-zinc-300 mb-1" for="priorYearTax">
          ...
          <input
            ...
            class="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 text-zinc-100 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-indigo-500"
          />
          <p class="text-xs text-zinc-500 mt-1">Enables safe harbor calculation</p>
        </div>
    ...
    <div v-if="estimate" class="mt-6 space-y-4">
      <!-- Safe harbor banner -->
      <div v-if="safeHarborAmount" class="bg-indigo-500/10 border border-indigo-500/30 rounded-xl p-4 text-sm">
        <p class="font-medium text-indigo-200 mb-1">Safe Harbor: {{ fmt(safeHarborAmount / 4) }} / quarter</p>
      ...
      <div class="bg-zinc-900 rounded-xl border border-zinc-800 p-6">
        <h2 class="text-lg font-semibold text-zinc-100 mb-4">2026 Quarterly Payment Schedule</h2>
        <div class="space-y-3">
          <div
          ...
          >
            <div>
              <p class="font-medium text-zinc-100 text-sm">{{ q.label }}</p>
              <p class="text-xs text-zinc-500">Due {{ q.due }}</p>
            </div>
            <p class="font-semibold text-zinc-100">{{ fmt(q.amount) }}</p>
          </div>
        </div>
        <div class="border-t border-zinc-800 mt-4 pt-4 flex justify-between font-semibold text-zinc-100">
      ...
      <div class="bg-zinc-900 rounded-xl border border-zinc-800 p-6">
        <h2 class="text-lg font-semibold text-zinc-100 mb-3">Tax Breakdown</h2>
        <div class="space-y-2 text-sm">
          ...
          <div v-if="estimate.result.state.tax > 0" class="flex justify-between text-zinc-400">
            <span>State Income Tax</span>
            <span>{{ fmt(estimate.result.state.tax) }}</span>
          </div>
          <div class="flex justify-between text-zinc-600 text-xs pt-1">
